...
@property (weak, nonatomic) IBOutlet UIButton *notificationToSend;
...
- (IBAction)sendNotificationButtonPressed:(id)sender {
    NSLog(@"send31H");
    DACommand *daCommand = [[DACommand alloc] init];
    NSData *data = [daCommand sendNotifyConnect];
    NSLog(@"send31H data: %@", data);
    [[EADSessionController sharedController] writeData:data];
}
